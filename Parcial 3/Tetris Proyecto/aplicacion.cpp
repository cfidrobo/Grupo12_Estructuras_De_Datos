...
void Aplicacion::iniciar() {
	...
	marquee.ejecutar("Universidad de las fuerzas armadas ESPE - tetris");
	login.inicializar();
...
void Aplicacion::generarReporte() {
	...
	inSrc << "<html><body><h1>Tetris</h1><br>";
	inSrc << "<div>";
	inSrc << "<strong>Numeros del 1 al 9 en el programa: </strong><code>" << inorden.str() << "</code><br>";
	inSrc << "<div>";
	inSrc << "<div>";
	inSrc << "<strong>1-2-3-5-6-7-8 </strong><code>" << inorden.str() << "</code><br>";
	inSrc << "</div><br><br>";
	inSrc << "</body></html>";
	inFile << inSrc.str();
	inFile.close();
